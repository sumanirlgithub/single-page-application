...
import com.neo.api.order.events.PaymentAuthorizationPendingEvent;
...
public class OrderService {
    ...
    private static final Duration AUTH_TIMEOUT = Duration.ofSeconds(2);
...
//ToDo:
//    public Mono<OrderResponse> placeOrderUsingWebFlux(OrderRequestPayload request) {
//
//        PurchaseOrder order = convertOrderRequestToEntity(request);
//        order.setOrderStatus(OrderStatus.CREATED);
//        //order.setCustomerId(customer.getCustomerId());
//        orderJpaRepository.save(order);
//
//        // request.payment() returns a PaymentRequest object — clean, immutable, contains only the fields needed by the Payment Service.
//        return paymentClient.authorize(request.payment())
//                .timeout(AUTH_TIMEOUT)
//                .flatMap(result -> handleImmediateResult(order, result))
//                .onErrorResume(TimeoutException.class, ex ->
//                        handleTimeout(order)
//                );
//    }
//
//    private Mono<OrderResponse> handleImmediateResult(
//            PurchaseOrder order,
...
//            orderJpaRepository.save(order);
//
//            // Continue async workflow
//            publisher.publishEvent(new PaymentAuthorizedEvent(order));
//
//            return Mono.just(OrderResponse.confirmed(order));
//        }
//
//        order.markPaymentFailed(result.reason());
//        orderJpaRepository.save(order);
//
//        return Mono.just(OrderResponse.failed(order));
//    }
//
//    private Mono<OrderResponse> handleTimeout(Order order) {
//
//        // Payment still running asynchronously
//        publisher.publishEvent(new PaymentAuthorizationPendingEvent(order));
//
//        return Mono.just(
//                OrderResponse.processing(order)
//        );
//    }
